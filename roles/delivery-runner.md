...
- Before launching `dev-impl`, the runner must make the batch/implementation-context patch-ready: target files, edit anchors, service seams, return shape, render insertion point, test assertions, forbidden data exposure, search budget, validation commands, and first-progress expectations must be explicit. If a field is not applicable, record `N/A` with a reason; if the runner cannot decide the seam/shape/assertions, escalate to lead instead of sending an ambiguous batch to impl.
