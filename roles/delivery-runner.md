...
- `fast` root delivery may use `scripts/lead-launch-runner.sh --delivery-profile fast --allow-fast-degraded --ack-fast-shared-workdir-risk ...` only for local/reversible work in an exclusive checkout. The launcher records foreground/current-workdir fallbacks, residual risk, and optional-doctor tooling debt in root state; `standard` and `guarded` never use this bypass.
