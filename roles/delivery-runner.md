...
- Before root launch, use `scripts/check-delivery-capabilities.sh --delivery-profile <fast|standard|guarded> --doctor-mode <required|optional|not-adopted> --json` or the equivalent launcher check. A `blocked` result stops delivery; `degraded` may proceed only through its listed, explicitly approved fallback.
- `fast` root delivery may use `scripts/lead-launch-runner.sh --delivery-profile fast --allow-fast-degraded ...` only for local/reversible work. The launcher records foreground/current-workdir fallbacks in root state; `standard` and `guarded` never use this bypass.
