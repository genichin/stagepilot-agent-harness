...
- Before significant impl work for a PR-bound `guarded` kickoff, the runner should already have completed publication auth preflight in the delivery worktree and escalated immediately on `publication_auth_missing`. This is not a default `fast` or `standard` gate; escalate when publication/release-sensitive risk enters scope.
