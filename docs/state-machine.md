...
### done
The assigned unit of work has completed its expected execution path and produced evidence. For a `lead -> delivery-runner` root kickoff item, `done` is the required completion signal that returns active ownership to the lead for release-stage review.
...
For runner-owned delivery completion, the lead-visible `done` delivery-state transition is mandatory; a separate completion notification/summary is optional unless a project overlay explicitly requires one.
