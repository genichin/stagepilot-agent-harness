...
- The default downstream launch path is explicit runner-owned fresh child worker execution: `scripts/runner-launch-impl.sh <impl_handoff_artifact> <delivery_state>` and `scripts/runner-launch-qc.sh <qc_handoff_artifact> <delivery_state>` for short/simple bounded work.
- Every `delivery-runner -> dev-impl` and `delivery-runner -> dev-qc` handoff must start a fresh child execution session. Workers must not rely on prior chat/session context; retries and rework must receive prior attempt evidence only through explicit artifacts such as handoff, implementation-context, progress, final-result, logs, state, and verification documents.
