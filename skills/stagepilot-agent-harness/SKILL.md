...
- A delivery `done` transition is a merge-ready hand-back, never proof of release/deployment/milestone completion. The only positive lifecycle kinds are `release_readiness`, `release_completion`, `milestone_completion` (including program completion), and `deployment_readiness`; each must pass `scripts/record_lifecycle_claim.py`. The writer binds an accepted claim to the snapshot artifact/digest/source revisions, and on any unavailable, stale, conflicting, malformed, mismatched, or unsupported input records only immutable `unverified` / `BLOCKED` evidence. See `../../docs/control-plane-snapshots.md`.
