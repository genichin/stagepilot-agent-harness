...
- `dev-impl` and `dev-qc` are intentionally distinct; QC is not an extension of implementation. Each runner→impl/QC handoff starts a fresh child execution session; previous attempts are passed only through artifacts, not inherited chat context.
