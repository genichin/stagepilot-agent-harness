...
- Artifact creation alone does not start execution. The lead explicitly launches `delivery-runner`. The default launcher path prepares an isolated per-kickoff git worktree/branch and starts a detached background `tmux` session, but a `fast` kickoff may use the explicit `--allow-fast-degraded` foreground/current-workdir fallback when the state records the selected fallback and residual risk.
