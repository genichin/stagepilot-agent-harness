...
- [Evidence-backed control-plane snapshots](docs/control-plane-snapshots.md) are the separate, provider-neutral source of truth for positive release/deployment/milestone claims. The lifecycle claim writer fails closed on unavailable, stale, conflicting, or mismatched evidence; delivery `done` remains only a hand-back to lead review.
...
- For `standard`/`guarded` independent QC, use `scripts/run_qc_rework_loop.py` rather than manually reenacting the same-scope QC FAIL loop. It launches fresh supervised QC, permits only classified same-scope implementation/evidence rework, runs supplied validation commands, and performs the fresh QC re-review. The controller persists `qc_rework_loop` state and blocks/escalates after the third unresolved verdict; see [state-machine.md](docs/state-machine.md#bounded-autonomous-qc-rework-loop) for the contract and invocation.
